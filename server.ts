  ...
  app.get('/api/data', async (req, res) => {
    try {
      ...
      if (!data || Object.keys(data).length === 0 || (!data.products && !data.customers)) {
        ...
        const [pRes, cRes, compRes, saleRes, tplRes, instRes, purRes, movRes] = await Promise.all([
        ...
        if (
          ...
          data = {
            ...
            basketTemplates: (tplRes.data || []).map((t: any) => ({
              id: t.id,
            ...
            sales: (saleRes.data || []).map((s: any) => ({
              ...
              paymentPlan: s.payment_plan || 'cash',
            ...
            purchases: (purRes.data || []).map((pu: any) => ({
              id: pu.id,
            ...
            stockMovements: (movRes.data || []).map((m: any) => ({
              id: m.id,
