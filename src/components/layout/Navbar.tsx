  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex w-64 bg-[#1e293b] text-white flex-col shrink-0 border-r border-[#334155] select-none">
        ...
        {/* Company Selector Widget in Sidebar */}
        <CompanySelector
          variant="sidebar"
          onOpenManager={() => setIsCompanyManagerOpen(true)}
        />
              ...
              <div className="flex items-center justify-between pb-4 border-b border-[#334155]">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-[#94a3b8] uppercase tracking-wider">SaaS Multibanco</p>
                  </div>
                </div>
                <button
                  type="button"
...
export const TopHeader: React.FC<NavProps> = ({ onOpenNewSale, onOpenMobileMenu }) => {
  const { activeTab, setActiveTab, summaryMetrics, activeCompany } = useApp();
  const { currentUser, logout, isSupabaseOnline } = useAuth();
  const [isCompanyManagerOpen, setIsCompanyManagerOpen] = useState(false);
    ...
    <>
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 lg:px-8 shrink-0 z-20">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          {/* Company Selector in Top Header */}
          <CompanySelector onOpenManager={() => setIsCompanyManagerOpen(true)} />

          {/* Supabase Status Chip */}
          <div
            title={isSupabaseOnline ? 'Supabase Conectado' : 'Supabase Configurado / Modo Local'}
            className="hidden xl:inline-flex items-center gap-1.5 px-2.5 py-1 bg-slate-100 border border-slate-200 rounded-lg text-[11px] font-semibold text-slate-700"
          >
